      ...
      <div className="hidden lg:flex lg:w-[480px] xl:w-[560px] lg:flex-col lg:justify-center lg:relative">
        <div className="relative z-10 p-8 space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {["직원관리", "급여계산", "세무처리", "매출분석", "수익창출"].map(tag => <span key={tag} className="px-3 py-1 bg-card/80 backdrop-blur-sm rounded-full text-sm text-foreground border border-border/50">
                {tag}
              </span>)}
          </div>
      ...
      <div className="w-full h-full lg:w-[480px] lg:flex-shrink-0">
        <div className="relative flex h-full max-w-xl flex-col bg-background shadow-2xl lg:max-w-none mx-auto">
          <div className="flex-1 overflow-auto">
            {/* Header */}
            {showHeader && (
              <header className="sticky top-0 z-10 border-b bg-card/70 backdrop-blur-md px-4 py-3 pt-[calc(env(safe-area-inset-top)+12px)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    <div
                      className={`rounded-lg px-2 py-1 -my-1 transition-colors ${
                        !showBackButton ? "cursor-pointer hover:bg-muted -mx-2" : ""
                      }`}
