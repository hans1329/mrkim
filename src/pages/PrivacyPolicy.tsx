...
import { privacyTranslations } from "@/data/privacyPolicyTranslations";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
      ...
      <main className="container max-w-3xl mx-auto px-4 py-8">
        <div className="prose prose-sm dark:prose-invert max-w-none space-y-8">
          ...
          {t.sections.map((section, i) => (
            <section key={i}>
              {section.title && <h2 className="text-lg font-semibold border-b pb-2">{section.title}</h2>}
                    ...
                    <thead>
                      <tr className="bg-muted/50">
                        {section.table.headers.map((h, j) => (
                          <th key={j} className="border px-3 py-2 text-left">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
              ...
              {section.contact && (
                <div className="mt-3 p-4 bg-muted/30 rounded-lg">
                  <h3 className="font-medium">{section.contact.title}</h3>
                  <ul className="mt-2 space-y-1">
                    {section.contact.items.map((item, j) => <li key={j}>{item}</li>)}
                  </ul>
                </div>
              )}
            </section>
