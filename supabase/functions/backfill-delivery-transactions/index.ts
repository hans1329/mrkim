...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const order of orders) {
      ...
      if (totalAmt > 0) {
        txRows.push({
          ...
          external_tx_id: `${prefix}_${order.order_no}`,
      ...
      for (const f of fees) {
        ...
        txRows.push({
          ...
          source_type: "delivery",
          source_name: sourceName,
          external_tx_id: `${prefix}_${order.order_no}_${f.suffix}`,
          transaction_date: txDate,
          transaction_time: txTime,
          amount: amt,
          type: "expense",
    ...
    console.log(`Generated ${txRows.length} transaction rows`);
